# mongo_to_oracle is (c) 2021 Adel Ramezani <adramazany at gmail.com>.
# The mongo_to_oracle module was contributed to Python as of Python 3.8 and thus
# was licensed under the Python license. Same license applies to all files in
# the mongo_to_oracle package project.
    ...
    if len(df)>0 :
        ...
        for col in df.columns :
            val={'original':col
                ...
                ,'isnumeric':(col in df_columns_numeric_medians and df_columns_numeric_medians[col]['median'] is not np.NAN)
                ,'isfloat':  (col in df_columns_numeric_medians and df_columns_numeric_medians[col]['median'] is not np.NAN
                              and myisnumeric(df_columns_numeric_medians[col]['max']) and (float(df_columns_numeric_medians[col]['max'])%1>0) )
                 }
...
def new_column_sql_definition(col,clean_columns):
    ...
    col_type=' varchar2('+str(col_val['length'])+' CHAR)'
    if col_val['isnumeric']:
        col_type=' numeric('+str(col_val['length'])
        if col_val['isfloat']:
            col_type+=",3"
        col_type+=')'
    return col+col_type
...
def add_jalali_dates(data,timestamp_column_names,timezone=pytz.timezone('Asia/Tehran')):
    for col in timestamp_column_names :
        if col in data:
            data['jd_'+col] = data[col].apply(lambda x: str(jdatetime.date.fromtimestamp(float(x)/1000).astimezone(timezone).strftime('%Y%m%d')) if x and str(x).lower()!='nan' else x )
##################################################################################
        ...
        for subtable in subtables:
            ...
            for subtable_row in subtable_df.values.tolist():
                subtable_insert_query = "insert into "+subtable["tablename"]+"(" \
                                        ...
                                        +") values ('" \
                                        +("', '").join(str(v).replace("'"," ") for v in subtable_row) \
                                        + "')"
    ...
    try:
        ...
        # for i,row in df_without_subtables.iterrows():
            insert_query = "insert into "+dest_table_name+"(" \
                           ...
                           +("', '").join(row) \
                           + "')"
            oracle_cursor.execute(insert_query.replace("'nan'","null"))
            counter+=1
