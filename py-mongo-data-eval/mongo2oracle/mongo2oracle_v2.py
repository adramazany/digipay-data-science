...
##################################################################################
def _etl_update_between_lastmodified_lastcreated():
    pass
##################################################################################
def _etl_incremental(mongoDB,mongo_collection
    ...
    result=[0]
    while counter>0 :
        query="SELECT MAX("+dest_column_4max+") ,max(" \
        +str(mongo_modification_column if mongo_modification_column else dest_column_4max) \
        ...
        if counter==0:
            mongo_query={mongo_ordered_column:{"$gt":max_ordered_value}}
            print("etl_incremental: query=%s => max_ordered_value=%s, mongo_query=%s"%(query, max_ordered_value,mongo_query))
...
def etl_incremental(mongo_uri,mongo_db,mongo_collection
    ...
    _etl_incremental(mongoDB,mongo_collection,oracleDB,dest_table_name,dest_column_type,dest_column_4max,mongo_ordered_column,mongo_modification_column,chunk_size,fn_clean_data,fn_cnv_max_value,timestamp2jalali,timestamp2gregorian,subtables)
